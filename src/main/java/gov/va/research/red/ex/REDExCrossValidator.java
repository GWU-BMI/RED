...
public class REDExCrossValidator implements CrossValidatable {
	...
	public static void main(String[] args) throws IOException, ConfigurationException {
		if (args.length != 1) {
			...
			List<CVResult> results = rexcv.crossValidate(vttfiles, labels, folds, allowOvermatches, caseInsensitive,
					holdouts, useTier2, stopAfterFirstFold.booleanValue(), shuffle, limit);
	...
	List<CVResult> crossValidate(List<File> vttFiles, String label, int folds, Boolean allowOvermatches,
			Boolean caseInsensitive, List<String> holdouts, Boolean useTier2, Boolean stopAfterFirstFold,
			Boolean shuffle, int limit) throws IOException {
		Collection<String> labels = new ArrayList<>(1);
		labels.add(label);
		return crossValidate(vttFiles, labels, folds, allowOvermatches, caseInsensitive, holdouts, useTier2,
	 ...
	 */
	List<CVResult> crossValidate(List<File> vttFiles, Collection<String> labels, int folds, Boolean allowOverMatches,
			Boolean caseInsensitive, List<String> holdouts, Boolean useTier2, Boolean stopAfterFirstFold,
			Boolean shuffle, int limit) throws IOException, FileNotFoundException {
		...
		LOG.info("Cross validating " + snippets.size() + " snippets from " + vttFiles + " files.\n" + "Folds: " + folds
				...
				+ "\nshuffle: " + shuffle + "\nsnippet.limit: " + limit);
		return crossValidateSnippets(snippets, labels, folds, allowOverMatches, caseInsensitive, holdouts, useTier2,
				stopAfterFirstFold, shuffle, limit);
	}
